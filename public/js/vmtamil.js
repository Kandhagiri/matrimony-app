if (typeof keyman === 'undefined') {
    console.log('Keyboard requires KeymanWeb 10.0 or later');
    if (typeof tavultesoft !== 'undefined')
        tavultesoft.keymanweb.util.alert("This keyboard requires KeymanWeb 10.0 or later");
} else {
    KeymanWeb.KR(new Keyboard_vm_tamil_typewriter());
}
function Keyboard_vm_tamil_typewriter() {
    this._v = (typeof keyman != "undefined" && typeof keyman.version == "string") ? parseInt(keyman.version, 10) : 9;
    this.KI = "Keyboard_vm_tamil_typewriter";
    this.KN = "Tamil Typewriter (Visual Media)";
    this.KMINVER = "10.0";
    this.KV = {
        F: ' 1em "Noto Sans Tamil"',
        K102: 0
    };
    this.KV.KLS = {
        "shift": ["'", "ஸ", "\"", "%", "ஜ", "ஶ", "ஷ", "&", "*", "(", ")", "ஸ்ரீ", "+", "", "", "", "", "று", "நு", "சு", "கூ", "லு", "ரு", "ஐ", "டீ", "ீ", "ூ", "ௌ", "௸", "", "", "", "", "ளு", "னு", "கு", "ழு", "ழ", "து", "மு", "டு", "`", "ஞ", "", "", "", "", "", "", "ஷ", "ஓ", "ஊ", "ஏ", "க்ஷ", "சூ", "ஆ", "ஈ", "?", "-", "", "", "", "", "", ""],
        "default": ["ஃ", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "/", "=", "", "", "", "ணு", "ற", "ந", "ச", "வ", "ல", "ர", "ை", "டி", "ி", "ு", "ஹ", "க்ஷ", "", "", "", "ய", "ள", "ன", "க", "ப", "ா", "த", "ம", "ட", "்", "ங", "", "", "", "", "", "", "ண", "ஒ", "உ", "எ", "ெ", "ே", "அ", "இ", ",", ".", "", "", "", "", "", ""]
    };
    this.KV.BK = (function(x) {
        var e = Array.apply(null, Array(65)).map(String.prototype.valueOf, ""), r = [], v, i, m = ['default', 'shift', 'ctrl', 'shift-ctrl', 'alt', 'shift-alt', 'ctrl-alt', 'shift-ctrl-alt'];
        for (i = m.length - 1; i >= 0; i--)
            if ((v = x[m[i]]) || r.length)
                r = (v ? v : e).slice().concat(r);
        return r
    }
    )(this.KV.KLS);
    this.KDU = 1;
    this.KH = "Visual Media Tamil Typewriter layout";
    this.KM = 0;
    this.KBVER = "1.2.1";
    this.KMBM = 0x10;
    this.KVKL = {
        "tablet": {
            "displayUnderlying": false,
            "layer": [{
                "id": "default",
                "row": [{
                    "id": "1",
                    "key": [{
                        "id": "K_1",
                        "text": "1"
                    }, {
                        "id": "K_2",
                        "text": "2"
                    }, {
                        "id": "K_3",
                        "text": "3"
                    }, {
                        "id": "K_4",
                        "text": "4"
                    }, {
                        "id": "K_5",
                        "text": "5"
                    }, {
                        "id": "K_6",
                        "text": "6"
                    }, {
                        "id": "K_7",
                        "text": "7"
                    }, {
                        "id": "K_8",
                        "text": "8"
                    }, {
                        "id": "K_9",
                        "text": "9"
                    }, {
                        "id": "K_0",
                        "text": "0"
                    }, {
                        "id": "K_HYPHEN",
                        "text": "/"
                    }, {
                        "id": "K_EQUAL",
                        "text": "="
                    }, {
                        "id": "K_BKSP",
                        "text": "*BkSp*",
                        "width": "100",
                        "sp": "1"
                    }]
                }, {
                    "id": "2",
                    "key": [{
                        "id": "K_Q",
                        "text": "ணு",
                        "pad": "75"
                    }, {
                        "id": "K_W",
                        "text": "ற"
                    }, {
                        "id": "K_E",
                        "text": "ந"
                    }, {
                        "id": "K_R",
                        "text": "ச"
                    }, {
                        "id": "K_T",
                        "text": "வ"
                    }, {
                        "id": "K_Y",
                        "text": "ல"
                    }, {
                        "id": "K_U",
                        "text": "ர"
                    }, {
                        "id": "K_I",
                        "text": "ை"
                    }, {
                        "id": "K_O",
                        "text": "டி"
                    }, {
                        "id": "K_P",
                        "text": "ி"
                    }, {
                        "id": "K_LBRKT",
                        "text": "ு"
                    }, {
                        "id": "K_RBRKT",
                        "text": "ஹ"
                    }, {
                        "id": "T_new_136",
                        "width": "10",
                        "sp": "10"
                    }]
                }, {
                    "id": "3",
                    "key": [{
                        "id": "K_BKQUOTE",
                        "text": "ஃ"
                    }, {
                        "id": "K_A",
                        "text": "ய"
                    }, {
                        "id": "K_S",
                        "text": "ள"
                    }, {
                        "id": "K_D",
                        "text": "ன"
                    }, {
                        "id": "K_F",
                        "text": "க"
                    }, {
                        "id": "K_G",
                        "text": "ப"
                    }, {
                        "id": "K_H",
                        "text": "ா"
                    }, {
                        "id": "K_J",
                        "text": "த"
                    }, {
                        "id": "K_K",
                        "text": "ம"
                    }, {
                        "id": "K_L",
                        "text": "ட"
                    }, {
                        "id": "K_COLON",
                        "text": "்"
                    }, {
                        "id": "K_QUOTE",
                        "text": "ங"
                    }, {
                        "id": "K_BKSLASH",
                        "text": "க்ஷ"
                    }]
                }, {
                    "id": "4",
                    "key": [{
                        "id": "K_SHIFT",
                        "text": "*Shift*",
                        "width": "160",
                        "sp": "1",
                        "nextlayer": "shift"
                    }, {
                        "id": "K_Z",
                        "text": "ண"
                    }, {
                        "id": "K_X",
                        "text": "ஒ"
                    }, {
                        "id": "K_C",
                        "text": "உ"
                    }, {
                        "id": "K_V",
                        "text": "எ"
                    }, {
                        "id": "K_B",
                        "text": "ெ"
                    }, {
                        "id": "K_N",
                        "text": "ே"
                    }, {
                        "id": "K_M",
                        "text": "அ"
                    }, {
                        "id": "K_COMMA",
                        "text": "இ"
                    }, {
                        "id": "K_PERIOD",
                        "text": ","
                    }, {
                        "id": "K_SLASH",
                        "text": "."
                    }, {
                        "id": "T_new_162",
                        "width": "10",
                        "sp": "10"
                    }]
                }, {
                    "id": "5",
                    "key": [{
                        "id": "K_LOPT",
                        "text": "*Menu*",
                        "width": "140",
                        "sp": "1"
                    }, {
                        "id": "K_SPACE",
                        "width": "930"
                    }, {
                        "id": "K_ENTER",
                        "text": "*Enter*",
                        "width": "145",
                        "sp": "1"
                    }]
                }]
            }, {
                "id": "shift",
                "row": [{
                    "id": "1",
                    "key": [{
                        "id": "K_1",
                        "text": "ஸ"
                    }, {
                        "id": "K_2",
                        "text": "\""
                    }, {
                        "id": "K_3",
                        "text": "%"
                    }, {
                        "id": "K_4",
                        "text": "ஜ"
                    }, {
                        "id": "K_5",
                        "text": "ஶ"
                    }, {
                        "id": "K_6",
                        "text": "ஷ"
                    }, {
                        "id": "K_7",
                        "text": "&"
                    }, {
                        "id": "K_8",
                        "text": "*"
                    }, {
                        "id": "K_9",
                        "text": "("
                    }, {
                        "id": "K_0",
                        "text": ")"
                    }, {
                        "id": "K_HYPHEN",
                        "text": "ஸ்ரீ"
                    }, {
                        "id": "K_EQUAL",
                        "text": "+"
                    }, {
                        "id": "K_BKSP",
                        "text": "*BkSp*",
                        "width": "100",
                        "sp": "1"
                    }]
                }, {
                    "id": "2",
                    "key": [{
                        "id": "K_Q",
                        "pad": "75"
                    }, {
                        "id": "K_W",
                        "text": "று"
                    }, {
                        "id": "K_E",
                        "text": "நு"
                    }, {
                        "id": "K_R",
                        "text": "சு"
                    }, {
                        "id": "K_T",
                        "text": "கூ"
                    }, {
                        "id": "K_Y",
                        "text": "லு"
                    }, {
                        "id": "K_U",
                        "text": "ரு"
                    }, {
                        "id": "K_I",
                        "text": "ஐ"
                    }, {
                        "id": "K_O",
                        "text": "டீ"
                    }, {
                        "id": "K_P",
                        "text": "ீ"
                    }, {
                        "id": "K_LBRKT",
                        "text": "ூ"
                    }, {
                        "id": "K_RBRKT",
                        "text": "ௌ"
                    }, {
                        "id": "T_new_245",
                        "width": "10",
                        "sp": "10"
                    }]
                }, {
                    "id": "3",
                    "key": [{
                        "id": "K_BKQUOTE",
                        "text": "'"
                    }, {
                        "id": "K_A"
                    }, {
                        "id": "K_S",
                        "text": "ளு"
                    }, {
                        "id": "K_D",
                        "text": "னு"
                    }, {
                        "id": "K_F",
                        "text": "கு"
                    }, {
                        "id": "K_G",
                        "text": "ழு"
                    }, {
                        "id": "K_H",
                        "text": "ழ"
                    }, {
                        "id": "K_J",
                        "text": "து"
                    }, {
                        "id": "K_K",
                        "text": "மு"
                    }, {
                        "id": "K_L",
                        "text": "டு"
                    }, {
                        "id": "K_COLON",
                        "text": "`"
                    }, {
                        "id": "K_QUOTE",
                        "text": "ஞ"
                    }, {
                        "id": "K_BKSLASH",
                        "text": "௸"
                    }]
                }, {
                    "id": "4",
                    "key": [{
                        "id": "K_SHIFT",
                        "text": "*Shift*",
                        "width": "160",
                        "sp": "2",
                        "nextlayer": "default"
                    }, {
                        "id": "K_Z",
                        "text": "ஷ"
                    }, {
                        "id": "K_X",
                        "text": "ஓ"
                    }, {
                        "id": "K_C",
                        "text": "ஊ"
                    }, {
                        "id": "K_V",
                        "text": "ஏ"
                    }, {
                        "id": "K_B",
                        "text": "க்ஷ"
                    }, {
                        "id": "K_N",
                        "text": "சூ"
                    }, {
                        "id": "K_M",
                        "text": "ஆ"
                    }, {
                        "id": "K_COMMA",
                        "text": "ஈ"
                    }, {
                        "id": "K_PERIOD",
                        "text": "?"
                    }, {
                        "id": "K_SLASH",
                        "text": "-"
                    }, {
                        "id": "T_new_271",
                        "width": "10",
                        "sp": "10"
                    }]
                }, {
                    "id": "5",
                    "key": [{
                        "id": "K_LOPT",
                        "text": "*Menu*",
                        "width": "140",
                        "sp": "1"
                    }, {
                        "id": "K_SPACE",
                        "width": "930"
                    }, {
                        "id": "K_ENTER",
                        "text": "*Enter*",
                        "width": "145",
                        "sp": "1"
                    }]
                }]
            }]
        }
    };
    this.KVER = "17.0.333.0";
    this.KVS = [];
    this.gs = function(t, e) {
        return this.g0(t, e);
    }
    ;
    this.gs = function(t, e) {
        return this.g0(t, e);
    }
    ;
    this.g0 = function(t, e) {
        var k = KeymanWeb
          , r = 0
          , m = 0;
        if (k.KKM(e, 16400, 49)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஸ");
            }
        } else if (k.KKM(e, 16400, 222)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஞ");
            }
        } else if (k.KKM(e, 16400, 51)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "%");
            }
        } else if (k.KKM(e, 16400, 52)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஜ");
            }
        } else if (k.KKM(e, 16400, 53)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஶ");
            }
        } else if (k.KKM(e, 16384, 222)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ங");
            }
        } else if (k.KKM(e, 16400, 57)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "(");
            }
        } else if (k.KKM(e, 16400, 48)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, ")");
            }
        } else if (k.KKM(e, 16400, 187)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "+");
            }
        } else if (k.KKM(e, 16384, 188)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "இ");
            }
        } else if (k.KKM(e, 16384, 189)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "/");
            }
        } else if (k.KKM(e, 16384, 190)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, ",");
            }
        } else if (k.KKM(e, 16384, 191)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, ".");
            }
        } else if (k.KKM(e, 16384, 48)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "0");
            }
        } else if (k.KKM(e, 16384, 49)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "1");
            }
        } else if (k.KKM(e, 16384, 50)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "2");
            }
        } else if (k.KKM(e, 16384, 51)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "3");
            }
        } else if (k.KKM(e, 16384, 52)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "4");
            }
        } else if (k.KKM(e, 16384, 53)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "5");
            }
        } else if (k.KKM(e, 16384, 54)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "6");
            }
        } else if (k.KKM(e, 16384, 55)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "7");
            }
        } else if (k.KKM(e, 16384, 56)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "8");
            }
        } else if (k.KKM(e, 16384, 57)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "9");
            }
        } else if (k.KKM(e, 16400, 186)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "`");
            }
        } else if (k.KKM(e, 16384, 186)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "்");
            }
        } else if (k.KKM(e, 16400, 188)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஈ");
            }
        } else if (k.KKM(e, 16384, 187)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "=");
            }
        } else if (k.KKM(e, 16400, 190)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "?");
            }
        } else if (k.KKM(e, 16400, 191)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "-");
            }
        } else if (k.KKM(e, 16400, 50)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "\"");
            }
        } else if (k.KKM(e, 16400, 65)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
            }
        } else if (k.KKM(e, 16400, 66)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "க்ஷ");
            }
        } else if (k.KKM(e, 16400, 67)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஊ");
            }
        } else if (k.KKM(e, 16400, 68)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "னு");
            }
        } else if (k.KKM(e, 16400, 69)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "நு");
            }
        } else if (k.KKM(e, 16400, 70)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "கு");
            }
        } else if (k.KKM(e, 16400, 71)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ழு");
            }
        } else if (k.KKM(e, 16400, 72)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ழ");
            }
        } else if (k.KKM(e, 16400, 73)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஐ");
            }
        } else if (k.KKM(e, 16400, 74)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "து");
            }
        } else if (k.KKM(e, 16400, 75)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "மு");
            }
        } else if (k.KKM(e, 16400, 76)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "டு");
            }
        } else if (k.KKM(e, 16400, 77)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஆ");
            }
        } else if (k.KKM(e, 16400, 78)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "சூ");
            }
        } else if (k.KKM(e, 16400, 79)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "டீ");
            }
        } else if (k.KKM(e, 16400, 80)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ீ");
            }
        } else if (k.KKM(e, 16400, 81)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
            }
        } else if (k.KKM(e, 16400, 82)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "சு");
            }
        } else if (k.KKM(e, 16400, 83)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ளு");
            }
        } else if (k.KKM(e, 16400, 84)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "கூ");
            }
        } else if (k.KKM(e, 16400, 85)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ரு");
            }
        } else if (k.KKM(e, 16400, 86)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஏ");
            }
        } else if (k.KKM(e, 16400, 87)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "று");
            }
        } else if (k.KKM(e, 16400, 88)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஓ");
            }
        } else if (k.KKM(e, 16400, 89)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "லு");
            }
        } else if (k.KKM(e, 16400, 90)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஷ");
            }
        } else if (k.KKM(e, 16384, 219)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ு");
            }
        } else if (k.KKM(e, 16384, 220)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "க்ஷ");
            }
        } else if (k.KKM(e, 16384, 221)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஹ");
            }
        } else if (k.KKM(e, 16400, 54)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஷ");
            }
        } else if (k.KKM(e, 16400, 189)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஸ்ரீ");
            }
        } else if (k.KKM(e, 16384, 192)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஃ");
            }
        } else if (k.KKM(e, 16384, 65)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ய");
            }
        } else if (k.KKM(e, 16384, 66)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ெ");
            }
        } else if (k.KKM(e, 16384, 67)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "உ");
            }
        } else if (k.KKM(e, 16384, 68)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ன");
            }
        } else if (k.KKM(e, 16384, 69)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ந");
            }
        } else if (k.KKM(e, 16384, 70)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "க");
            }
        } else if (k.KKM(e, 16384, 71)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ப");
            }
        } else if (k.KKM(e, 16384, 72)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ா");
            }
        } else if (k.KKM(e, 16384, 73)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ை");
            }
        } else if (k.KKM(e, 16384, 74)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "த");
            }
        } else if (k.KKM(e, 16384, 75)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ம");
            }
        } else if (k.KKM(e, 16384, 76)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ட");
            }
        } else if (k.KKM(e, 16384, 77)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "அ");
            }
        } else if (k.KKM(e, 16384, 78)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ே");
            }
        } else if (k.KKM(e, 16384, 79)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "டி");
            }
        } else if (k.KKM(e, 16384, 80)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ி");
            }
        } else if (k.KKM(e, 16384, 81)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ணு");
            }
        } else if (k.KKM(e, 16384, 82)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ச");
            }
        } else if (k.KKM(e, 16384, 83)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ள");
            }
        } else if (k.KKM(e, 16384, 84)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "வ");
            }
        } else if (k.KKM(e, 16384, 85)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ர");
            }
        } else if (k.KKM(e, 16384, 86)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "எ");
            }
        } else if (k.KKM(e, 16384, 87)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ற");
            }
        } else if (k.KKM(e, 16384, 88)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ஒ");
            }
        } else if (k.KKM(e, 16384, 89)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ல");
            }
        } else if (k.KKM(e, 16384, 90)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ண");
            }
        } else if (k.KKM(e, 16400, 219)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ூ");
            }
        } else if (k.KKM(e, 16400, 220)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "௸");
            }
        } else if (k.KKM(e, 16400, 221)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "ௌ");
            }
        } else if (k.KKM(e, 16400, 192)) {
            if (1) {
                r = m = 1;
                k.KDC(0, t);
                k.KO(-1, t, "'");
            }
        }
        return r;
    }
    ;
}
