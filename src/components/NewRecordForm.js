...
import TamilTextFieldTest from './TamilTextFieldTest';
import TamilTextFieldS from './TamilTextFieldS';
import TranslatableTextField from './TranslatableTextField';
import ImageUpload from './ImageUpload';
import HoroscopeForm from './HoroscopeForm';
import { formatDateForStorage, formatDateInput, isValidDate } from '../utils/dateUtils';
...
const NewRecordForm = ({ onNavigate, onShowSnackbar }) => {
  ...
  const handleSave = async () => {
    ...
    // Convert date to storage format and trim strings
    const dataToSave = {
      ...formData,
      ...
      EducationDetails: formData.EducationDetails ? formData.EducationDetails.trim() : '',
      Occupation: formData.Occupation ? formData.Occupation.trim() : '',
      OccupationDetails: formData.OccupationDetails ? formData.OccupationDetails.trim() : '',
    ...
    try {
      const result = await window.electronAPI.addProfile(sanitizedData);
      if (result.success && result.data && result.data.ProfileID) {
        onShowSnackbar('சுயவிவரம் வெற்றிகரமாக சேமிக்கப்பட்டது', 'success');
        // Navigate to view profile page after successful save
        setTimeout(() => {
          onNavigate('view', result.data.ProfileID);
        }, 500); // Small delay to show success message
      } else {
        onShowSnackbar(result.error || 'பிழை: சுயவிவரத்தை சேமிக்க முடியவில்லை', 'error');
        setIsSaving(false);
      }
    } catch (error) {
